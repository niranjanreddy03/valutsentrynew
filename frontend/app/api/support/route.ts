...
export async function POST(request: NextRequest) {
  ...
  const parsed = await readJsonBody<SupportPayload>(request, 32 * 1024)
  ...
  if (subject.length > 160 || message.length > 5000) {
    return NextResponse.json(
      { error: 'Support ticket is too large' },
      { status: 400 },
    )
  }
  ...
  if (slackWebhook && ticket.tier === 'premium_plus') {
    try {
      const slackUrl = new URL(slackWebhook)
      if (slackUrl.protocol !== 'https:') throw new Error('Slack webhook must be HTTPS')
